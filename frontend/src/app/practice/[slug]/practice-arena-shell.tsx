    ...
    <div className="mx-auto grid w-full max-w-[1400px] gap-10 rounded-[44px] border border-sky-500/22 bg-slate-950/90 p-8 shadow-[0_0_80px_rgba(56,189,248,0.22)] backdrop-blur-2xl sm:p-10 xl:grid-cols-[1.3fr_1fr]">
      <article className="space-y-8">
        ...
        <div className="rounded-2xl border border-sky-500/20 bg-slate-900/70 p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-sky-400/60">Test cases</p>
          <div className="mt-3 flex flex-wrap gap-2">
            {safeTestcases.map((testcase, index) => {
              const status = statusForIndex(index);
              const isActive = index === activeTestcaseIndex;
              const baseStyles = "rounded-full border px-3 py-1 text-[11px] uppercase tracking-[0.35em] transition";
              const activeStyles = isActive
                ? "border-sky-400 text-sky-100"
                : "border-sky-500/30 text-sky-200/70 hover:border-sky-400/50";
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            <div className="rounded-xl border border-sky-500/25 bg-slate-950/75 p-3">
              <p className="text-[11px] uppercase tracking-[0.35em] text-sky-400/60">Input</p>
              <pre className="mt-3 max-h-40 overflow-auto text-sm text-sky-100/85 whitespace-pre-wrap">
                {activeTestcase?.input ?? ""}
              </pre>
            </div>
            <div className="rounded-xl border border-sky-500/25 bg-slate-950/75 p-3">
              <p className="text-[11px] uppercase tracking-[0.35em] text-sky-400/60">Expected output</p>
              <pre className="mt-3 max-h-40 overflow-auto text-sm text-emerald-200/85 whitespace-pre-wrap">
      ...
      <section className="flex flex-col gap-6">
        ...
        <div className="rounded-[32px] border border-slate-700/60 bg-[#050b18] p-6 shadow-[0_0_48px_rgba(56,189,248,0.2)]">
          ...
          <div className="mt-3 space-y-4 text-sm text-sky-100/85">
            ...
            <div className="rounded-2xl border border-slate-700/45 bg-[#030713] p-4">
                ...
                {safeTestcases.map((testcase, index) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => setActiveTestcaseIndex(index)}
                      className={`flex items-center gap-2 rounded-full border px-3 py-1 text-[11px] uppercase tracking-[0.3em] transition ${
              ...
              <div className="mt-4 grid gap-3 text-xs text-sky-100/85">
                ...
                <div>
                  ...
                  <pre className="mt-1 max-h-36 overflow-auto whitespace-pre-wrap rounded-lg border border-slate-700/50 bg-[#040915] p-3 text-sm text-sky-100/90">
                ...
                <div>
                  ...
                  <pre className="mt-1 max-h-36 overflow-auto whitespace-pre-wrap rounded-lg border border-slate-700/50 bg-[#040915] p-3 text-sm text-emerald-200/85">
                    {activeTestcase?.output ?? ""}
                  </pre>
                </div>
                {activeResult?.stderr && (
                  <div>
                    ...
                    <pre className="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded-lg border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-200/85">
                      {activeResult.stderr}
                    </pre>
                  </div>
                )}
